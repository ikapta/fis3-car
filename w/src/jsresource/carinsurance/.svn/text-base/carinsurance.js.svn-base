$(function () {
    ...
    var carinsurance = {
        ...
        varlidate() {
            ...
            if (!company) {
                VALIDATEUTIL.showErrorMsg('请选择保险公司');
                return false;
            }
            if (!productName) {
            ...
            if (!time) {
             ...
             if (!carPrice) {
                VALIDATEUTIL.showErrorMsg('请输入车险价格');
                return false;
            }
        ...
        priceOpreation() {
            var _this = this;
            var $priceDom = $('.inputGroup input');
            $('.plus').click(function () {
                var price = $('.inputGroup input').val();
                price = parseInt(price) + _this.minAdd;
                $priceDom.val(price);
            })
            $('.minus').click(function () {
                var price = $('.inputGroup input').val();
                if (price > _this.minInvest) {
                    price = parseInt(price) - _this.minAdd;
                    $priceDom.val(price);
                }
            })
            $priceDom.blur(function () {
                var price = $('.inputGroup input').val();
                if (!/^-?[1-9]\d*$/.test(price) || price < _this.minInvest) {
                    $priceDom.val(_this.minInvest);
                    price = _this.minInvest;
                    return;
                }
            })
        }
